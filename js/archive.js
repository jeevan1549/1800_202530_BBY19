...
auth.onAuthStateChanged((user) => {
  ...
  const archiveRef = collection(db, "users", user.uid, "archivedTasks");
    ...
    snapshot.forEach((docSnap) => {
      ...
      div.innerHTML = `
        ...
        <ul class="taskDetails">
          ...
          <p class="taskDetail"><strong>Description:</strong> ${
            task.taskDesc
          }</p>
          <p class="taskDetail"><strong>Final Points:</strong> ${
            task.finalPoints
          }</p>
          <p class="taskDetail"><strong>Final EXP:</strong> ${task.finalExp}</p>
        </ul>
        <div class="taskFooter">
          <p class="timeArchived"><small>Archived: ${task.archivedAt
            .toDate()
      ...
      div.querySelector(".deleteBtn").addEventListener("click", async () => {
        await deleteDoc(
          doc(db, "users", user.uid, "archivedTasks", docSnap.id)
        );
      });
